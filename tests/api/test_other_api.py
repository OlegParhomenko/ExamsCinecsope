...
from db_models.db_transaction_models import AccountTransactionTemplate
...
class TestOtherAPI:
    ...
    def test_accounts_transaction_template(self, db_session: Session):
        ...
        try:
            ...
            with allure.step('обновляем балансы для последующей проверки'):
                stan_refresh = db_session.query(AccountTransactionTemplate).filter_by(user=stan.user).one()
                bob_refresh = db_session.query(AccountTransactionTemplate).filter_by(user=bob.user).one()
            with allure.step('обновляем балансы для последующей проверки'):
                assert stan_refresh.balance == 1000
                assert bob_refresh.balance == 500
